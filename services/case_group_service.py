...
class CaseGroupService:
    ...
    def update(group_id: int, user, name: Optional[str] = None, parent_id: Optional[int] = None):
        ...
        if parent_id is not None:
            if parent_id == 0:
                ...
                if not new_parent:
                    raise BizError(404, "新的父分组不存在")
                if new_parent.department_id != group.department_id:
                    raise BizError(400, "新的父分组不在同一部门")
                # 循环检测：不能移动到自身或后代
                descendant_ids = CaseGroupRepository.get_descendant_ids_inclusive(group)
                if new_parent.id in descendant_ids:
                    raise BizError(400, "不能将分组移动到其自身或子分组下")

        # 校验同级重名
        target_parent_id = new_parent.id if new_parent else None
        target_name = name if name is not None else group.name
        if CaseGroupRepository.exists_name_under_parent(group.department_id, target_parent_id, target_name, exclude_id=group.id):
            raise BizError(400, "同级已存在同名分组")

        old_path = group.path
        changed = CaseGroupRepository.update_group_parent_and_name(group, new_parent, name)

        if changed:
            # 重新计算自身 path
            group.path = CaseGroupService._build_path(new_parent, group.name)
    ...
    def copy(group_id: int, user, target_parent_id: Optional[int] = None, new_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if CaseGroupRepository.exists_name_under_parent(
            ...
            raise BizError(400, "目标父分组下已存在同名分组")
        ...
        with db.session.begin():
            ...
            old_id_to_new[source_group.id] = new_root.id
            ...
            for g in all_groups:
                if g.id == source_group.id:
                    continue
                old_parent_id = g.parent_id
                new_parent_id = old_id_to_new.get(old_parent_id)
                if new_parent_id is None:
                    # 理论上不会
                    raise BizError(500, "复制结构异常：父分组未找到映射")

                new_parent = CaseGroupRepository.get_by_id(new_parent_id)
                clone = CaseGroupRepository.create(
                    department_id=g.department_id,
                    parent_id=new_parent.id,
                    name=g.name,
                    path=f"{new_parent.path}/{g.name}",
                    order_no=g.order_no,
                    created_by=user.id,
                    updated_by=user.id
                )
                old_id_to_new[g.id] = clone.id
                created_groups.append(clone)

            db.session.flush()  # 确保 new group ids 可用
            ...
            for g in all_groups:
                ...
                for tc in tc_list:
                    new_group_id = old_id_to_new[g.id]
        ...
        return {
            "new_root_group_id": old_id_to_new[source_group.id],
    ...
    def list_children(department_id: int, user, parent_id: Optional[int], with_case_count: bool = False):
        ...
        if parent_id:
            parent = CaseGroupRepository.get_by_id(parent_id)
            if not parent:
                raise BizError(404, "父分组不存在")
            if parent.department_id != department_id:
                raise BizError(400, "父分组不在该部门")
